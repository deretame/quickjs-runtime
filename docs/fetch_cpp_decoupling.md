...
| TLS 取循环的暗道 | `qjs::current_io()` / `io_of(ctx)` | TLS 反查 Runtime 的 io——只在绑定层存在，核心库不得引入 |
...
2. **调度器注入**：核心库实例构造时接受外部 `boost::asio::io_context&` 作为调度器，不持有、不新建、无全局/TLS 依赖；
   随处可建实例，多实例可共用一个 io（各自独立的中间件/TLS/代理配置互不干扰）。
...
- 不做请求侧流式上传（`duplex`）；请求 body 仍是整收 `std::string`。
...
| body 提取（string/ArrayBuffer/URLSearchParams/Blob/FormData → 字节 + Content-Type） | 绑定层 | 物化为 `Request::body` + 头（现状即如此） |
...
- [ ] 文档同步：`fetch_design.md` / `known_issues.md` / `README.md` 更新。

...
function fail(name, err) {
  ...
  g.__cheerio_tests.failures.push({
    name,
    message: String(err && err.message ? err.message : err) + ' [e=' + (err === null ? 'null' : typeof err) + ']',
...
function it(name, fn) {
  ...
  // 调试：IT_FILTER 环境变量（runner 注入）只跑匹配的用例
  if (g.__it_filter && String(name).indexOf(g.__it_filter) === -1) return;
  // 调试：IT_LIMIT 只跑前 N 个用例（二分泄漏用）
  g.__it_count = (g.__it_count || 0) + 1;
  if (g.__it_limit && g.__it_count > g.__it_limit) return;
  try {
  ...
  if (v === null) out = 'null';
    ...
    if (seen.has(v)) {
      ...
      if (Array.isArray(v)) {
        ...
        out = '[' + items.join(', ') + ']';
      } else if (v !== null && v !== undefined && (v.cheerio || (v.length !== undefined && v.type === undefined))) {
        // cheerio 类数组
        const items = [];
        for (let i = 0; i < v.length; i++) items.push(formatValue(v[i], seen));
